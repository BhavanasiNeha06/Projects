...
# Function to repeat the traffic signal cycle with dynamic green light timings
def repeat():
    ...
    # Adjust the green light duration based on the vehicle count
    vehicle_count = vehicles[directionNumbers[currentGreen]]['crossed']
    adjusted_green_time = calculateGreenLightTime(vehicle_count)  # Calculate green light duration based on vehicle count
    
    signals[currentGreen].green = adjusted_green_time  # Set the new green signal time
    ...
    signals[nextGreen].red = signals[currentGreen].yellow + signals[currentGreen].green  # Adjust next red light
    
    repeat()  # Recursive call to repeat the cycle

# Function to calculate the green light duration based on the number of vehicles
def calculateGreenLightTime(vehicle_count):
    # Set base green light time (for instance, 10 seconds)
    base_green_time = 10
    # Increase the green light time for each vehicle in the direction
    green_time_increase_per_vehicle = 2  # Increase time by 2 seconds for each vehicle
    
    # Calculate the total green time based on vehicle count
    adjusted_green_time = base_green_time + (vehicle_count * green_time_increase_per_vehicle)
    
    # Optionally, limit the max green time
    max_green_time = 60  # Maximum green light time
    if adjusted_green_time > max_green_time:
        adjusted_green_time = max_green_time
